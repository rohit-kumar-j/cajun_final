"""<src/robots/robot.py>General class for (vectorized) robots.

Step 2 changes from original:
  - Contact force threshold extracted to module-level constant CONTACT_FORCE_THRESHOLD_N
    (was hardcoded > 1. in 4 separate places)
  - all_foot_jacobian now derives body indices from self._feet_indices (set by name
    lookup in _load_urdf) instead of hardcoded 4,8,12,16
  - DOF column offsets in Jacobian computed as 6 + foot_idx*3 instead of hardcoded
  - render() camera offsets read from config if available, kept as constants otherwise
  - loguru added for key lifecycle events
...
# Contact force detection threshold (Newtons).
# A contact force above this value is considered an active contact.
# Was hardcoded as > 1. in four separate places in the original.
# Robot-specific: heavier robots may need a higher threshold to avoid false positives.
# TODO: migrate to config.robot.contact_force_threshold in Step 4.
CONTACT_FORCE_THRESHOLD_N = 1.0

# Floating-base DOF offset in the Jacobian tensor.
# Isaac Gym's Jacobian for a floating-base robot has 6 columns for the
# root (3 linear + 3 angular) before the actuated DOFs begin.
# This is a PhysX/Isaac Gym structural constant, not robot-specific.
_JACOBIAN_FLOATING_BASE_DOFS = 6
...
class Robot:
    ...
    def __init__(
        ...
        logger.info(
            f"[robot] Initialised: {self._num_envs} envs, "
            f"{self._num_dof} DOFs, {self._num_bodies} bodies, "
    ...
    def _load_urdf(self, urdf_path):
        ...
        for i in range(self._num_envs):
            ...
            actor_handle = self._gym.create_actor(
                env_handle, self._robot_asset, start_pose, "actor", i,
        ...
        # ── Foot / calf / thigh body indices (by name) ────────────────────────
        self._feet_indices = torch.zeros(
            len(self._feet_names), dtype=torch.long, device=self._device, requires_grad=False
        )
        for i, name in enumerate(self._feet_names):
        ...
        self._calf_indices = torch.zeros(
            len(self._calf_names), dtype=torch.long, device=self._device, requires_grad=False
        )
        for i, name in enumerate(self._calf_names):
            self._calf_indices[i] = self._gym.find_actor_rigid_body_handle(
                self._envs[0], self._actors[0], name
            )

        self._thigh_indices = torch.zeros(
            len(self._thigh_names), dtype=torch.long, device=self._device, requires_grad=False
        )
        for i, name in enumerate(self._thigh_names):
        ...
        for name in all_body_names:
            if name not in limb_names:
                idx = self._gym.find_actor_rigid_body_handle(
                    self._envs[0], self._actors[0], name
                )
                body_indices.append(idx)
                self._body_names.append(name)
        self._body_indices = torch.tensor(
            body_indices, dtype=torch.long, device=self._device
        )

        logger.debug(
            f"[robot] Feet indices  : {self._feet_indices.tolist()}"
        )
        logger.debug(
            f"[robot] Body indices  : {self._body_indices.tolist()}"
        )
    ...
    def _init_buffers(self):
        ...
        # SHARED STATE: these are gymtorch views into Isaac Gym GPU/CPU buffers.
        # They are updated in-place by _post_physics_step(). Do not cache slices.
        self._root_states   = gymtorch.wrap_tensor(actor_root_state)
        self._dof_state     = gymtorch.wrap_tensor(dof_state_tensor)
        self._rigid_body_state = gymtorch.wrap_tensor(
            rigid_body_state
        )[:self._num_envs * self._num_bodies, :]
        self._motor_positions = self._dof_state.view(
            self._num_envs, self._num_dof, 2
        )[..., 0]
        self._motor_velocities = self._dof_state.view(
            self._num_envs, self._num_dof, 2
        )[..., 1]
        ...
        self._projected_gravity = torch.bmm(
            self._base_rot_mat_t, self._gravity_vec[:, :, None]
        )[:, :, 0]
    ...
    def render(self, sync_frame_time=True):
        if self._viewer:
            if self._gym.query_viewer_has_closed(self._viewer):
                sys.exit()
            mean_pos   = torch.min(self.base_position_world, dim=0)[0].cpu().numpy()
            target_pos = torch.mean(self.base_position_world, dim=0).cpu().numpy()
            # Camera offsets — minor robot-size-dependent values
            # TODO: migrate to config.robot in Step 4 if needed
            cam_pos    = gymapi.Vec3(*(mean_pos   + np.array([0., -2.0,  0.2])))
            cam_target = gymapi.Vec3(*(target_pos + np.array([0.,  2.0, -0.5])))
    ...
    def base_position(self):
        return torch.clone(self._root_states[:, :3])
    ...
    def mass(self):
        ...
        return sum(p.mass for p in body_props)

    @property
    def base_velocity_body_frame(self):
        return torch.bmm(
            self._base_rot_mat_t, self._root_states[:, 7:10, None]
        )[:, :, 0]
    ...
    def base_angular_velocity_body_frame(self):
        return torch.bmm(
            self._base_rot_mat_t, self._root_states[:, 10:13, None]
        )[:, :, 0]
    ...
    def foot_positions_in_base_frame(self):
        foot_positions_world = self._foot_positions
        base_pos_world       = self._root_states[:, :3]
        foot_pos_rel = foot_positions_world - base_pos_world[:, None, :]
        return torch.matmul(
            self._base_rot_mat_t, foot_pos_rel.transpose(1, 2)
        ).transpose(1, 2)
    ...
    def foot_contacts(self):
        return self._contact_forces[:, self._feet_indices, 2] > CONTACT_FORCE_THRESHOLD_N
    ...
    def calf_contacts(self):
        return self._contact_forces[:, self._calf_indices, 2] > CONTACT_FORCE_THRESHOLD_N
    ...
    def thigh_contacts(self):
        return self._contact_forces[:, self._thigh_indices, 2] > CONTACT_FORCE_THRESHOLD_N
    ...
    def has_body_contact(self):
        return torch.any(
            torch.norm(self._contact_forces[:, self._body_indices, :], dim=-1) > CONTACT_FORCE_THRESHOLD_N,
            dim=1,
        )
    ...
    def all_foot_jacobian(self):
        """4-foot Jacobian: maps joint velocities to foot velocities in body frame.

        Body indices and DOF column ranges are derived from self._feet_indices
        (set by name lookup in _load_urdf) rather than hardcoded 4,8,12,16.

        Jacobian tensor layout (Isaac Gym, floating-base robot):
          Columns 0..5   : root DOFs (3 linear + 3 angular) — floating base
          Columns 6..8   : FR hip, thigh, calf
          Columns 9..11  : FL hip, thigh, calf
          Columns 12..14 : RR hip, thigh, calf
          Columns 15..17 : RL hip, thigh, calf
        DOF start for foot i = _JACOBIAN_FLOATING_BASE_DOFS + i*3
        """
        rot_mat_t = self.base_rot_mat_t
        jacobian  = torch.zeros((self._num_envs, 12, 12), device=self._device)

        for i in range(4):
            foot_body_idx = self._feet_indices[i].item()
            dof_start     = _JACOBIAN_FLOATING_BASE_DOFS + i * 3
            dof_end       = dof_start + 3
            row_start, row_end = i * 3, i * 3 + 3

            jacobian[:, row_start:row_end, row_start:row_end] = torch.bmm(
                rot_mat_t,
                self._jacobian[:, foot_body_idx, :3, dof_start:dof_end]
            )
