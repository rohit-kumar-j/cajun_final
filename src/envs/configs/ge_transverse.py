...
def get_config():
  ...
  gait_config.gait_name = "g0_transverse"
  gait_config.stepping_frequency = 2.0
  # Correct Order:[ FrontRight, FrontLeft, RearRight, RearLeft ]
  gait_config.initial_offset = np.array([0.,0.18,.5,.68], # transverse
                                        dtype=np.float32) * (2 * np.pi)
  gait_config.swing_ratio = np.array([0.68, 0.68, 0.68, 0.68], dtype=np.float32)
