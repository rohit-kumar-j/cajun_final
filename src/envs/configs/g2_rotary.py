...
def get_config():
    ...
    gait_config.initial_offset    = np.array(
        [0., 0.1, 0.57, 0.47], dtype=np.float32
    ) * (2 * np.pi)
    ...
    robot_config.urdf_path = "data/go1/urdf/go1.urdf"  # switched to Go1 to match baseline
    ...
    robot_config.init_height = 0.268
    ...
    raibert_config.desired_base_height    = 0.26    # matches original RaibertSwingLegController default
    ...
    qp_config.body_mass    = 13.076   # Go1 mass (matches go1.urdf; was 15.018 for Go2)
    ...
    qp_config.desired_body_height = 0.26

    # Friction and contact
    qp_config.foot_friction_coef = 0.6
    ...
    rewards_config.min_stepping_freq_reward = 1.5   # stepping_freq_reward breakeven (Hz)
