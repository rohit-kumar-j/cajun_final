"""<src/controllers/phase_gait_generator.py>"""
import copy
...
class PhaseGaitGenerator:
    ...
    def __init__(self, robot: Any, gait_config: ConfigDict):
        ...
        # ── Read config via cfg() to track warnings ───────────────────────────
        _w = {}
        C = "PhaseGaitGenerator"
        
        freq   = cfg(gait_config, "stepping_frequency", REQUIRED, C, _w)
        offset = cfg(gait_config, "initial_offset",     REQUIRED, C, _w)
        ratio  = cfg(gait_config, "swing_ratio",        REQUIRED, C, _w)
        
        flush_warnings(gait_config, "config.gait", _w)
        
        gait_name = getattr(gait_config, "gait_name", "custom")
        logger.info(f"[gait] Initialised '{gait_name}' generator (freq={freq} Hz)")
        
        # ── Internal config mapping ───────────────────────────────────────────
        self._config = ConfigDict()
        self._config.initial_offset = to_torch(offset, device=self._device)
        self._config.swing_ratio    = to_torch(ratio, device=self._device)
        self._config.stepping_frequency = freq
    ...
    def reset(self):
        ...
        ) * 2 * torch.pi * (1 - self._config.swing_ratio)
    ...
    def update(self):
        ...
        self._current_phase += (
            2 * torch.pi * self._stepping_frequency[:, None] * delta_t[:, None]
        )
    ...
    def desired_contact_state_se(self):
        ...
        raw_contact = torch.where(
            torch.logical_and(
                modulated_phase > self._swing_cutoff, modulated_phase < 2. * torch.pi
            ),
            False,
            True
        )
