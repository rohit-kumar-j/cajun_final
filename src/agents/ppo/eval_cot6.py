...
def add_text_to_frame(frame_path, sim_time):
    """Add simulation time overlay to a frame image."""
    try:
...
def save_data_on_exit(reason="unknown"):
    ...
    if _exit_state['data_count'] > 0 and _exit_state['data_arrays'] is not None:
        try:
            ...
            # Generate COT plot after data is saved
            create_cot_plot(_exit_state['output_dir'], _exit_state['gait_name'])
...
def main(argv):
    ...
    print(f"{'Step':<8} {'SimTime':<10} {'RealTime':<10} {'CurrVel':<10} {'DesVel':<10}")
    print("="*60)
    
    steps_count = 0
    data_count = 0
    velocity_index = 0
    sim_dt = 0.002  # 500 Hz simulation
    frames_per_step = max(1, int((1.0 / FLAGS.render_fps) / sim_dt))  # Capture every N steps to match FPS
    ...
    try:
        with torch.inference_mode():
            while steps_count < FLAGS.max_steps:
                ...
                # Video recording - capture frame every N steps to match desired FPS
                if FLAGS.record_video and (steps_count % frames_per_step == 0):
                    try:
                        ...
                        # Add sim time overlay to frame
                        add_text_to_frame(frame_path, t)
                ...
                # Print useful info every step
                curr_vel = np.linalg.norm(base_vel[:2])  # Horizontal velocity magnitude
                des_vel = velocity_schedule[velocity_index].item()
                real_time = time.time() - _exit_state['start_time']
                
                print(f"{steps_count:<8} {t:<10.3f} {real_time:<10.3f} {curr_vel:<10.3f} {des_vel:<10.3f}", end='\r')
