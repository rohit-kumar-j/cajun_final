...
from src.envs.configs import test
...
def get_training_config():
  ...
  # Increased from [512, 256, 128] to wider and deeper
  policy_config.actor_hidden_dims = [1024, 1024, 512, 256]
  policy_config.critic_hidden_dims = [1024, 1024, 512, 256]
  ...
  alg_config.learning_rate = 1e-3  # May want to reduce to 5e-4 for larger network
  ...
  runner_config.save_interval = 10
  runner_config.experiment_name = "test"
  runner_config.max_iterations = 16000
...
def get_config():
  ...
  config.environment = test.get_config()
  return config
